...
export const getEmployeeTable = async (roles: Role[]): Promise<EmployeeWithManagerID[]> => Promise.all(((await connectionPool.execute("SELECT * FROM employee"))[0] as {id: number, first_name: string, last_name: string, role_id: number, manager_id: number}[])
...
export const getEmployeesView = async (): Promise<EmployeeWithManagerName[]> => Promise.all(((await connectionPool.execute("SELECT * FROM employees_view"))[0] as {id: number, first_name: string, last_name: string, title: string, department: string, salary: number, manager: string}[])
